Ext.define('${jsAppNamespace}.view.accesslog.UaGraph', {
	extend: 'Ext.panel.Panel',
	alias: 'widget.uagraph',
	itemId: 'uaGraphPanel',

	requires: [ 'Ext.chart.axis.Category', 'Ext.chart.axis.Numeric',
			'Ext.chart.series.Bar', '${jsAppNamespace}.store.AccessLogUserAgents',
			'${jsAppNamespace}.store.AccessLogYears' ],
	...
	initComponent: function() {
		...
		me.items = [ {
			...
			animate: true,
			defaultInsets: 30,
			store: Ext.create('${jsAppNamespace}.store.AccessLogUserAgents'),
			legend: {
				position: 'right'
			},
			axes: [ {
		...
		me.dockedItems = [ {
			...
			items: [ {
				...
				labelWidth: 40,
				itemId: 'uaYearsCB',
				store: Ext.create('${jsAppNamespace}.store.AccessLogYears'),
				valueField: 'year',
				displayField: 'year',
				queryMode: 'local',
				forceSelection: true
