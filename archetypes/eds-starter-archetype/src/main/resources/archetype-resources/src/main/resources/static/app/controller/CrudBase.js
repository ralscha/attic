Ext.define('${jsAppNamespace}.controller.CrudBase', {
	...
	destroyObject: function(record) {
		...
		Ext.Msg.confirm(i18n.attention, this.destroyConfirmMsg(record), function(
			...
			if (buttonId === 'yes') {
				...
				store.sync({
					success: function() {
						${jsAppNamespace}.ux.window.Notification.info(i18n.successful,
	...
	onFormSaveButtonClick: function(button) {
		...
		store.sync({
			success: function(records, operation) {
				${jsAppNamespace}.ux.window.Notification.info(i18n.successful, me.successfulSaveMsg);
			},
			failure: function(records, operation) {
				store.rejectChanges();
			}
