Ext.define('${jsAppNamespace}.controller.Viewport', {
	extend: 'Deft.mvc.ViewController',
	requires: [ '${jsAppNamespace}.view.UserSettings' ],
	...
	onSettingsButtonClick: function() {
		if (${jsAppNamespace}.user) {
			var userSettingsWindow = Ext.create('${jsAppNamespace}.view.UserSettings');
			userSettingsWindow.down('form').loadRecord(
					Ext.create('${jsAppNamespace}.model.User', ${jsAppNamespace}.user));
	...
	onUserSettingsSaveButtonClick: function(button) {
		...
		userService.updateSettings(form.getForm().getFieldValues(), function(result) {
			if (result.success) {
				win.close();
				${jsAppNamespace}.ux.window.Notification.info(i18n.successful, i18n.settings_saved);
			}
		});
	},
