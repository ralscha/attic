...
Ext.define('CarTracker.controller.Cars', {
	...
	stores: [ 'Cars', 'Staff', 'CarImages', 'option.Makes', 'option.Models', 'option.Categories' ],
	views: [ 'car.List', 'car.edit.Form', 'car.edit.Window', 'car.search.Form', 'car.search.Window', 'car.Detail' ],
	refs: [ {
		ref: 'CarList',
	...
	init: function() {
		this.listen({
			...
			component: {
				...
				'form[xtype=car.edit.form] combobox[name=makeId]': {
					change: this.filterModel
				},
				'form[xtype=car.edit.form] combobox[name=modelId]': {
	...
	checkMake: function(queryPlan, eOpts) {
		var make = queryPlan.combo.up('form').down('[name=makeId]');
	...
	filterModel: function(combobox, newValue, oldValue, eOpts) {
		var model = combobox.up('form').down('[name=modelId]'), store = model.getStore(), filters = [ {
			property: 'make.id',
			value: newValue
		} ];
	...
	clearSearch: function(button, e, eOpts) {
		var me = this, grid = me.getCarList(), store = grid.getStore();
		// clear filter
	...
	showSearch: function(button, e, eOpts) {
		var me = this, win = me.getCarSearchWindow();
	...
	search: function(button, e, eOpts) {
		var me = this, win = me.getCarSearchWindow(), form = win.down('form'), grid = me.getCarList(), store = grid.getStore(), values = form
				.getForm().getFieldValues(), filters = [];
	...
	showContextMenu: function(view, record, item, index, e, eOpts) {
		...
		if (me.application.getWorkflowsController().hasWorkflowPermission(record.get('statusName'))) {
			// setup workflow actions
			switch (record.get('statusName')) {
			case 'In-Review':
			case 'In-Audit':
				items.push({
	...
	add: function(button, e, eOpts) {
		var me = this, record = Ext.create('CarTracker.model.Car', {
			carImageIds: []
		});
		// show window
	...
	save: function(button, e, eOpts) {
		var me = this, grid = me.getCarList(), store = grid.getStore(), win = button.up('window'), form = win.down('form'), record = form
				.getRecord(), values = form.getForm().getFieldValues(), view = me.getCarImageView().down('dataview'), callbacks;

		// get any images from image store
		values.carImageIds = view.getStore().collect('id');
		// set values of record from form
		record.set(values);
		// check if form is even dirty...if not, just close window and
		// stop everything...nothing to see here
		if (!record.dirty) {
			win.close();
			return;
		}
	...
	showEditWindow: function(record) {
		var me = this, win = me.getCarEditWindow(), isNew = record.phantom, data = [];
		// if window exists, show it; otherwise, create new instance
		if (!win) {
		...
		data = me.prepareImageData(record.get('carImageIds'));
	...
	showDetailWindow: function(record) {
		var me = this, win = me.getCarDetailWindow();
		// if window exists, show it; otherwise, create new instance
		if (!win) {
	...
	upload: function(button, e, eOpts) {
		var me = this, form = button.up('form'), image = form.down('[name=carImage]'), record = form.getRecord(), view = me.getCarImageView()
				.down('dataview'), uploadform = form.down('#uploadform');
		// validate upload
	...
	removeImage: function(record) {
		var me = this, dataview = me.getCarImageView().down('dataview'), store = dataview.getStore();
		// remove the image
		store.remove(record);
	},
	/**
	 * @private Prepares raw image path data for store
	 * @param {Array}
	 *            paths
	 */
