...
/*
    public int hashCode() {
        String str = new String(getType() + getGtf_number() + getCurrency() + getAmount() + 
    	             getAcct_mgmt_unit() + getAccount_number() + getExpiry_date() + 
    	             getGtf_proc_center());
    	return(str.hashCode());             
    }
  */  
    public boolean hasEmptyAccountNumbers() {
        if (getAccount_number().equals("") || getAcct_mgmt_unit().equals(""))  
            return(true);
        else
            return(false);        
    }
