...
public class MVS
{
    ...
    PreparedStatement urefNo, iWLstmt, sWLstmt, sW12stmt, uW12stmt,iW12stmt,
                       sW3stmt, uW3stmt, iW3stmt, sBLstmt, uBLstmt, iBLstmt;
    int refno;
    Random r = new Random();
        ...
        if (args.length >= 1)
        {
            if (args[0].equals("G"))
            {
				...
				Class.forName ("sun.jdbc.odbc.JdbcOdbcDriver");	
                    ...
                    String selectWord12 = "SELECT word1, word2, word3ref, total FROM Word12 "
            ...
            else
            {
                ...
                else
                {
               	...
               	String selectWord12   = "SELECT word1, word2, word3ref, total FROM Word12 "
               	                            + "WHERE word1 = ? AND word2 = ?";
               	String updateWord12   = "UPDATE Word12 SET total = ? "
               	                            + "WHERE word1 = ? AND word2 = ?";
               	String insertWord12   = "INSERT INTO Word12(word1, word2, word3ref, total) "
               	                            + "VALUES(?, ?, ?, ?)";
                String selectWord3    = "SELECT ref, word3, hits FROM Word3 "
                                            + "WHERE ref = ? AND word3 = ?";
                String updateWord3    = "UPDATE Word3 SET hits = ? "
                                            + "WHERE ref = ? AND word3 = ?";
                String insertWord3    = "INSERT INTO Word3(ref, word3, hits) "
                                            + "VALUES(?, ?, ?)";

                String selectBeginList= "SELECT word1, word2, total FROM BeginList "
                                            + "WHERE word1 = ? AND word2 = ?";
                String updateBeginList= "UPDATE BeginList SET total = ? "
                                            + "WHERE word1 = ? AND word2 = ?";
                String insertBeginList= "INSERT INTO BeginList(word1, word2, total) "
                                            + "VALUES(?, ?, ?)";
                ...
                String updateRefNo    = "UPDATE RefNo SET refno = ?";
      	        ...
      	        try
      	        {
                    ...
                    for (int i = 0; i < files.size(); i++)
                    {
                        ...
                        {
                            addWords((Integer)hash.elementAt(ix),
                                    (Integer)hash.elementAt(ix+1),
                            ...
                            if (ix == 0)
                            {
                                ...
                                if (Character.isUpperCase(ws.charAt(0)))
                                    addWordsToBeginList((Integer)hash.elementAt(ix), (Integer)hash.elementAt(ix+1));
                            }
                            else
                            {
                        ...
                        addWords((Integer)hash.elementAt(ix), (Integer)hash.elementAt(ix+1),
                                     endmark);
