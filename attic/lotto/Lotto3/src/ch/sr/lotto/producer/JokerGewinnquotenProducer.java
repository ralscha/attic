...
public class JokerGewinnquotenProducer extends Producer {
	...
	public JokerGewinnquotenProducer() {
  ...
  public void producePage(Database db, Properties props, AusspielungenStatistiken as) {
      ...
      JokerGainQuota[] jgqs = (JokerGainQuota[])db.get(JokerGainQuota.class, query);
      ...
      if (year > 0) {
        ...
        for (int i = 0; i < jgqs.length; i++) {
          writeJokerGewinnquote(pw, jgqs[i]);
        }
	...
	private void writeHead(PrintWriter pw, JokerGainQuota jgq) {
		pw.println("<br>");
	...
	private void writeJokerGewinnquote(PrintWriter pw, JokerGainQuota jgq) {
		pw.println("<tr>");
