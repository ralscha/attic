...
public final class CheckJokerAction extends Action {

  private static Category CAT = Category.getInstance(CheckJokerAction.class.getName());
      ...
      if ((jokers != null) && (jokers.length > 0)) {
        ...
        for (int i = fromno; i <= tono; i++) {
          ...
          for (int j = 0; j < jokers.length; j++) {      
            switch (draw.getJokerWin(jokers[j].getJoker())) {
        	    case 6 : win[4]++; break;
        	    case 5 : win[3]++; break;
        	    case 4 : win[2]++; break;
        	    case 3 : win[1]++; break;
