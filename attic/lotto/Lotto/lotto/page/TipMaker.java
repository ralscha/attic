...
public class TipMaker {
	...
	public java.util.List createTips(Ausspielungen ausspielungen, int numberOfTips) {
		...
		verbundMap = ausspielungen.getAusspielungenStatistik(AusspielungenType.ALLE).getVerteilungVerbund();
		hger = ausspielungen.getAusspielungenStatistik(AusspielungenType.ALLE).getVerteilungHaeufigkeitGerade();
		...
		int anzalle = ausspielungen.getAusspielungenStatistik(AusspielungenType.ALLE).getAnzahlAusspielungen();
		int anzaus42 = ausspielungen.getAusspielungenStatistik(AusspielungenType.AUS42).getAnzahlAusspielungen();
		int anzaus45 = ausspielungen.getAusspielungenStatistik(AusspielungenType.AUS45).getAnzahlAusspielungen();
		...
		for (int n = 0; n < numberOfTips; n++) {
			...
			do {
				...
				do {
					...
					//Testen
					if (zie.getZiehungStatistik().getGerade() != hgerauswahl)
						nichtgefunden = true;
					if (zie.getZiehungStatistik().getVerbund().equals(verbundauswahl))
						nichtgefunden = true;
					if ((summenauswahl - zie.getZiehungStatistik().getSumme() > 10) ||
    						(summenauswahl - zie.getZiehungStatistik().getSumme() < -10))
						nichtgefunden = true;
