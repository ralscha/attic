 ...
 * 
 * @author  Ralph Schaer
 * @version $Revision: 1.5 $ $Date: 2004/05/22 16:51:14 $
...
public class User extends Persistent {
   ...
   * @hibernate.collection-one-to-many  class="ch.ess.blank.db.UserConfiguration"
   */
  ...
  public static Iterator findWithSearchtext(String searchText, String roleId) throws HibernateException {
    ...
    if (!GenericValidator.isBlankOrNull(searchText)) {
      ...
      crit.add(Expression.or(Expression.or(Expression.like("email", searchText), Expression.like("name", searchText)),
          Expression.like("userName", searchText)));
    }

    if (!GenericValidator.isBlankOrNull(roleId)) {
      crit.createCriteria("userGroups").add(Expression.eq("id", new Long(roleId)));
    }
  ...
  public static User find(String userName, String passwordHash) throws HibernateException {
    List userList = HibernateSession.currentSession().find(
        "from User as u where u.userName = ? AND u.passwordHash = ?", new Object[]{userName, passwordHash},
  ...
  public static User find(String userName) throws HibernateException {
    List userList = HibernateSession.currentSession().find("from User as u where u.userName = ?", userName,
        Hibernate.STRING);
  ...
  public static User findWithLoginToken(String loginToken) throws HibernateException {
    List userList = HibernateSession.currentSession().find("from User as u where u.loginToken = ?", loginToken,
        Hibernate.STRING);
    if (userList.size() > 0) {
      return (User) userList.get(0);
    }
