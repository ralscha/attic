...
/** 
 * @author  Ralph Schaer
 * @version $Revision: 1.3 $ $Date: 2004/05/22 16:51:15 $ 
 * 
 * @struts.action path="/storeUserConfig" name="userConfigForm" input=".userconfig.edit" scope="session" validate="true" parameter="store" roles="userconfig"
 * @struts.action path="/editUserConfig" name="userConfigForm" input=".userconfig.edit" scope="session" validate="false" parameter="edit" roles="userconfig"
...
public class UserConfigEditAction extends DispatchAction {
  ...
  protected ActionForward store() throws Exception {
    ...
    if (!GenericValidator.isBlankOrNull(prefForm.getNewPassword())) {
      user.setPasswordHash(DigestUtils.shaHex(prefForm.getNewPassword()));
