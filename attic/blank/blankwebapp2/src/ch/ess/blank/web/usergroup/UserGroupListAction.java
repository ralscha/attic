...
/** 
 * @struts.action path="/listUserGroup" name="mapForm" scope="session" validate="false" roles="usergroup"
 * @struts.action-forward name="success" path=".userGroup.list"
 */
public class UserGroupListAction extends HibernateAction {

  private static DynaClass resultClass;

  static {
    resultClass = new BasicDynaClass("ResultClassUserGroup", null, new DynaProperty[]{
        new DynaProperty("id", Long.class), new DynaProperty("groupName", String.class),
  ...
  public ActionForward doAction() throws Exception {
    ...
    List resultList = UserGroup.find(searchText);
    ...
    while (resultIt.hasNext()) {
      UserGroup userGroup = (UserGroup) resultIt.next();
      DynaBean dynaBean = resultClass.newInstance();
      dynaBean.set("id", userGroup.getId());
      dynaBean.set("groupName", userGroup.getGroupName());
      dynaBean.set("canDelete", new Boolean(userGroup.canDelete()));
