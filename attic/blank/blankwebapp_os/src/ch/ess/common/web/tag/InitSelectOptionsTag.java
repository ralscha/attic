...
/**
 * @author  Ralph Schaer
 * @version $Revision: 1.4 $ $Date: 2004/05/22 12:24:41 $
 *  
...
public class InitSelectOptionsTag extends TagSupport {
  ...
  public int doStartTag() throws JspException {
    ...
    if (TagUtils.getInstance().lookup(pageContext, getId(), getScope()) == null) {
      ...
      try {
        ...
        options.setLocale(TagUtils.getInstance().getUserLocale(pageContext, null));
        options.setMessages(getResource());
        options.setRequest((HttpServletRequest) pageContext.getRequest());
        options.init();
  ...
  private MessageResources getResource() {
    return (MessageResources) pageContext.getRequest().getAttribute(Globals.MESSAGES_KEY);
  }
