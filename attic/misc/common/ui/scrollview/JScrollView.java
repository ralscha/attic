...
public class JScrollView extends JPanel implements ChangeListener, MouseListener,
MouseMotionListener {
	...
	public JScrollView(JScrollPane scrollpane, int size, boolean isWindow) {
		...
		double ratio = (double) childSize.height / (double) childSize.width;
	...
	public Rectangle calculateRectangle() {
		double xUnit = (double)(getSize().width - (getInsets().left + getInsets().right)) /
               		(double) total.width;
		double yUnit = (double)(getSize().height - (getInsets().top + getInsets().bottom)) /
               		(double) total.height;
	...
	public void paintComponent(Graphics g) {
		...
		if (scrollpane instanceof JScrollViewPane &&
    			(thumbnail == null || ((JScrollViewPane) scrollpane).capture.isDynamic)) {
			thumbnail = new BufferedImage(w, h, BufferedImage.TYPE_INT_RGB);
			Graphics gc = thumbnail.getGraphics();
			gc.drawImage(((JScrollViewPane) scrollpane).getImage(), 0, 0, w, h, this);
		}
		if (thumbnail != null)
	...
	public void mouseDragged(MouseEvent event) {
		...
		int xView = (int)(((double) x /
                   		(double)(getSize().width - (getInsets().top + getInsets().bottom))) *
                  		(double) total.width);
		int yView = (int)(((double) y /
                   		(double)(getSize().height - (getInsets().left + getInsets().right))) *
                  		(double) total.height);
