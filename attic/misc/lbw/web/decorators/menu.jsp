...
<head>
  ...
  <script type="text/javascript" src="<c:url value='/scripts/overlibmws/overlibmws.js'/>"></script>
  <decorator:head/>
<style type="text/css">
<!--
body {
...
<table border="0" cellspacing="0" cellpadding="0" class="c1">
  <tr>
    <td colspan="3" valign="middle" class="bigheader">&nbsp;<span style="cursor:help;" onmouseover="return overlib('<%=ch.ess.base.web.Infopanel.getTitleToolTipHtml()%>', FGCOLOR, '#EEEEEE', LEFT, WIDTH, 250, HEIGHT, 150);" onmouseout="return nd();">Lieferantenbewertung</span></td>
    <td width="1" align="right" rowspan="2"><img width="161" height="58" alt="Polytec Group" src="<c:url value='/images/logo.jpg'/>"></td>
  </tr>
    ...
    <c:if test="${not empty sessionScope.header_variables}" var="hasHeader">
      ...
      <td class="title" align="left">&nbsp;</td>
	   <td class="smalltext" align="right">
	     <bean:message key="user"/>: ${sessionScope.header_variables.name}&nbsp;&nbsp;	  
	   </td>
    </c:if>
    <c:if test="${not hasHeader}">
      <td colspan="3" class="title" align="left">&nbsp;</td>    
    </c:if>
