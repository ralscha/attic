...
import gtf.db.CONT_LIABILITY_TMP;
import gtf.db.FIRM_LIABILITY_TMP;
import gtf.db.CONT_LIABILITY_TMPTable;
import gtf.db.FIRM_LIABILITY_TMPTable;
import gtf.common.*;
...
public class LiabilityExporter implements gtf.common.Constants {

	private List firmList, contList;

	private void deleteContents(ServiceCenter sc) {
		try {
			CONT_LIABILITY_TMPTable cltt = new CONT_LIABILITY_TMPTable(sc.getConnection());
			FIRM_LIABILITY_TMPTable fltt = new FIRM_LIABILITY_TMPTable(sc.getConnection());
			Log.log(sc.getShortName() + " delete cont_Liability_tmp rows : " + cltt.delete());
			Log.log(sc.getShortName() + " delete firm_Liability_tmp rows : " + fltt.delete());
	...
	public static void main(String[] args) {
		...
		try {
			...
			if (args.length == 1) {
				...
				if (!scList.isEmpty())
					if (new LiabilityExporter().start(scList))
						rc = 0;
					else
						rc = 1;
				else
	...
	private void readData(Connection conn,PrintWriter out) throws SQLException, IOException {
		...
		CONT_LIABILITY_TMPTable cltt = new CONT_LIABILITY_TMPTable(conn);
		FIRM_LIABILITY_TMPTable fltt = new FIRM_LIABILITY_TMPTable(conn);
		...
		while(it.hasNext()) {
			CONT_LIABILITY_TMP contObj = (CONT_LIABILITY_TMP)it.next();
		...
		while(it.hasNext()) {
			FIRM_LIABILITY_TMP firmObj = (FIRM_LIABILITY_TMP)it.next();
