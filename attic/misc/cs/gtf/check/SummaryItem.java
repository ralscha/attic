...
public class SummaryItem implements Comparable {
	...
	private TBigDecimal totalSingle;
	private TBigDecimal totalTotal;
	...
	public TBigDecimal getTotal(String bkst, boolean fromTotal) {
		if (fromTotal) {
			return (TBigDecimal)totalAccountMap.get(bkst);			
		} else {
			return (TBigDecimal)singleAccountMap.get(bkst);
		}
	}
	...
	private void summarizeList(Iterator it, Map m) {
		...
		if (total.compareTo(TBigDecimal.ZERO) != 0) {
			m.put(mbkst, total);
		}
	}
	...
	public TBigDecimal getTotalSingle() {
		return totalSingle;
	}
	
	public TBigDecimal getTotalTotal() {
		return totalTotal;
	}

	public TBigDecimal getDifference() {
