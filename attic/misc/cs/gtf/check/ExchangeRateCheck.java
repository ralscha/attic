...
public class ExchangeRateCheck implements gtf.common.Constants {
	...
	public void check(List scList) {
		try {
			...
			while(it.hasNext()) {
				...
				if (!ok)
					sendMail(dateFormat.format(now.getTime()), sc.getShortName(), dateStr, noStr);
			}
		} catch (Exception e) {
	...
	private void sendMail(String nowStr, String center, String dateStr, String noStr) {
		String smtp = AppProperties.getStringProperty(P_SMTP_HOST);
		String sender = AppProperties.getStringProperty(P_FX_CHECK_MAIL_SENDER);
		List receiverList = AppProperties.getStringArrayProperty(P_FX_CHECK_MAIL_RECEIVER);
	...
	public static void main(String args[]) {
		...
		if (args.length == 1) {
			...
			if (!scList.isEmpty()) {
				new ExchangeRateCheck().check(scList);
			} 
		} 
	}
}
