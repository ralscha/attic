...
public class AccountFormat {
	...
	private final static COM.stevesoft.pat.Regex extAccountRegex = 
		new COM.stevesoft.pat.Regex("^(\\d{4})-[ ]*(\\d{1,7})-(\\d{2})(-(\\d{1,3})){0,1}$");
	private final static COM.stevesoft.pat.Regex intAccountRegex = 
		new COM.stevesoft.pat.Regex("^\\d{16}$");
	...
	public static String getExternString(String ibbb, String stamm, String p, 
		...
		if (last != null) {
			sb.append(Strings.rightJustify(String.valueOf(last), 3, '0'));		
		} else {
			sb.append("000");
		}
		/*
