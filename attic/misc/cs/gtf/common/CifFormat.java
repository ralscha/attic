...
public class CifFormat {
	...
	private final static COM.stevesoft.pat.Regex extCifRegex = 
		new COM.stevesoft.pat.Regex("^(\\d{4})-[ ]*(\\d{1,7})(-(\\d)){0,1}$");
	private final static COM.stevesoft.pat.Regex intCifRegex = 
		new COM.stevesoft.pat.Regex("^\\d{12}$");
	...
	public static String getExternString(String ibbb, String stamm, String p) {
		...
		sb.append(Strings.rightJustify(String.valueOf(stammno), 7, 
												 '0')).append("-");
		...
		if (p == null) {
			...
			sb.append(CheckSumCalculator.calc(temp.toString()));
		} else {
			sb.append(p);
		} 
	...
	public static String getInternString(String ibbb, String stamm, String p) {
		...
		if (p != null) {
			...
			sb.append(CheckSumCalculator.calc(sb.toString()));
		} 
