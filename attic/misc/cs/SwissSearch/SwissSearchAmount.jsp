...
function go1(){
if (document.searchform.requesttype.options[document.searchform.requesttype.selectedIndex].value == "http://159.122.248.165:8080/ss/servlet/SwissSearch") {
...
<input type="hidden" name="jsp.page" value="/SwissSearchAmount.jsp">
<table cellspacing="2" cellpadding="2" border="0">
<tr>
	<td><select name="requesttype" size="1" onchange="go1()">
	<option value="http://159.122.248.165:8080/ss/servlet/SwissSearch">Bank Reference</option>
	<option value="http://159.122.248.165:8080/ss/servlet/SwissSearch">Bank Name</option>
	<option value="amountbetween" selected>Dossier Amount</option></select></td>
	
    <td>From: <input type="text" name="requeststring" size="15"></td>
	<td>To: <input type="text" name="requeststring2" size="15"></td>
...
%>
	...
	<%
		String amountfrom = (String)request.getAttribute("ss.search.request.amount.from"); 
		String amountto = (String)request.getAttribute("ss.search.request.amount.to"); 
		if ((amountfrom != null) && (amountfrom.length() > 0)) {
		  if ((amountto != null) && (amountto.length() > 0)) {		  	
	%>
	Dossier Amount: From: <%= amountfrom %> To: <%= amountto %>
	<% } else { %>
	Dossier Amount: <%= amountfrom %>
	<% }} %>
   ...
   %>
		<tr>
			...
			<td align="right" class="text"><% String amount = row.getFormattedAmount(); if (amount.trim().equals("")) out.print("&nbsp;"); else out.print(amount); %></td>
		</tr>
	<% 
		} 
	%>
