...
<%
	today = Calendar.getInstance(local);	
...
function setOpenerDate(d, m, y)
{
	...
	window.opener.document.forms[0].<%=request.getParameter("elem")%>.value = dte;
...
<tr>
<td align="center"><font face="Arial,Helvetica,sans-serif" size="-1"><a href="calendar.jsp?form=<%=request.getParameter("form")%>&elem=<%=request.getParameter("elem")%>&m=<%= today.get(Calendar.MONTH) %>&y=<%= today.get(Calendar.YEAR) %>">Heute</a>: <%= dateFormat.format(today.getTime()) %></font></td>
</tr>
</table>
