...
/** A business entity class representing a TextResource
  ...
  * @hibernate.class  table="addressbookTextResource"
  */
public class TextResource extends Persistent {
  ...
  */
  ...
  /**      
   * @hibernate.map table="addressbookTextResourceLocale" 
   * @hibernate.collection-key column="textResourceId"    
   * @hibernate.collection-index column="locale" length="10" type="locale"
   * @hibernate.collection-element column="name" length="4000" not-null="true" type="string" 
   */
  ...
  public static List findName(String key) throws HibernateException {
    return HibernateSession.currentSession().find("from TextResource tr where tr.name = ?", key, Hibernate.STRING);
  }
  ...
  public static int delete(Long textResourceId) throws HibernateException {
    return HibernateSession.currentSession().delete(
      "from TextResource as tr where tr.id = ?",
      textResourceId,
      Hibernate.LONG);
  }

  public static String getTextResource(Locale locale, String na) throws HibernateException {

    List l = TextResource.findName(na);
    String text = "";
    if (!l.isEmpty()) {
