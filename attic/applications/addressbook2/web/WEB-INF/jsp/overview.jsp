...
<div id="overDiv" style="background-color: Silver; position: absolute; visibility: hidden; z-index: 1000;"></div>
...
<display:table name="resultab" export="false" scope="session" pagesize="<%= (String)session.getAttribute("static_noRows") %>" requestURI="<%= (String)pageContext.getAttribute("requestURI") %>" sort="list" defaultsort="2" class="list"  decorator="ch.ess.addressbook.web.contact.ContactDecorator">

  <display:column property="info" title="" align="center">  
  </display:column>
  ...
  <display:column property="export" title="" align="center">  
  </display:column>  
...
<misc:highlight tableNo="4" colNo="6" />
...
<input type="hidden" name="value(path)" value="/newContact.do" />
<input type="submit" name="go" value="<bean:message key="NewContact"/>" />
</html:form>
</td>
<td>&nbsp;&nbsp;&nbsp;</td>
<td>
<c:if test="${not empty sessionScope.resultab}">
<html:form action="/forward.do" method="post">
<input type="hidden" name="value(path)" value="/export.donogz" />
<input type="submit" name="go" value="<bean:message key="Exportxls"/>" />
</html:form>
</c:if>
</td>
</tr>
