...
/** A business entity class representing an User
  ...
  * @hibernate.class  table="abUser" proxy="ch.ess.addressbook.db.User"
  */
...
public class User extends Persistent {
  ...
  */
  ...
  public static Iterator findAll() throws HibernateException {
    return HibernateSession.currentSession().iterate("from User as u order by u.userName asc");  
  }
    
  public static Iterator findWithSearchtext(String searchText) throws HibernateException {
    searchText += "%";   
    return    
    HibernateSession.currentSession().iterate(
      "from User as u where u.email like ? or u.name like ? or u.userName like ? order by u.userName asc",
  ...
  public static User findWithLoginToken(String loginToken) throws HibernateException {
    List userList =
      HibernateSession.currentSession().find(
        "from User as u where u.loginToken = ?",
        loginToken,
        Hibernate.STRING);
    if (userList.size() > 0) {
      return (User)userList.get(0);
