package ch.ess.timetracker.web.userconfig;
...
public class UserConfigForm extends ValidatorForm {
  ...
  public String getHourRate() {
    if (hourRate != null) {
      return ch.ess.timetracker.Constants.DECIMAL_FORMAT.format(hourRate);    
    }
  ...
  /**   
   * @struts.validator type="required,integer"
   * @struts.validator-args arg0resource="userconfig.noRows"
   */
  ...
  public void fromForm(User user) throws HibernateException {
    ...
    WebContext.currentContext().getSession().setAttribute("static_noRows", config.getStringProperty(UserConfig.NO_ROWS, "10"));
