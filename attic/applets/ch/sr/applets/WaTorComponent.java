...
public class WaTorComponent extends Component implements Runnable {
	Image wtImg = null;
	Graphics wtG = null;
	Thread runner;
	Dimension d;
	...
	int fasten;
	int chrononen;
	WaTorChartComponent wtcc;
	WaTorStatusPanel wtsp;

	public WaTorComponent(int gx, int gy, int fbrut, int hbrut, int afisch, int ahai,
                      	int fasten, WaTorChartComponent wtcc, WaTorStatusPanel wtsp) {
		super();
		this.gx = gx;
		this.gy = gy;
		this.fbrut = fbrut;
	...
	private void createOffscreenImage() {
		if (getSize().width > 0) {
			...
			wtImg = createImage(getSize().width, getSize().height);
	...
	void clearImage() {
		if (wtImg != null) {
			...
			wtG.fillRect(0, 0, getSize().width, getSize().height);
			wtG.setColor(Color.black);
		}
	}
