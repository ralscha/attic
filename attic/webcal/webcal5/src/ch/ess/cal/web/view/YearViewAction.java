...
@SpringAction
@StrutsAction(path="/yearView", form=YearViewForm.class, roles="all", 
    input="/yearview.jsp", scope=ActionScope.REQUEST, validate=false)
public class YearViewAction extends FWAction implements CrumbInfo {
  ...
  public void doExecute(ActionContext ctx) throws Exception {
    ...
    YearViewForm yearForm = (YearViewForm)ctx.form();
    ...
    yearForm.setInputYear(String.valueOf(year));
    
    List<MonthBean> monthList = new ArrayList<MonthBean>();
    for (int month = Calendar.JANUARY; month <= Calendar.DECEMBER; month++) {
      MonthBean monthBean = monthBeanUtil.createMonthBean(locale, user, user, month, year, false, getResources(ctx.request()));
  ...
  public void increaseYear_onClick(FormActionContext ctx) throws Exception {
		YearViewForm form = (YearViewForm) ctx.form();
