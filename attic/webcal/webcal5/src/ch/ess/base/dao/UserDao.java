...
public class UserDao extends AbstractDao<User> {
  ...
  public User findByEmail(final String email) {
    ...
    criteria.createCriteria("emails").add(Restrictions.eq("email", email));
  ...
  public Set<String> getPermission(final String id) {
    ...
    for (UserUserGroup userUserGroup : userUserGroups) {
      ...
      for (UserGroupPermission userGroupPermission : userGroupPermissions) {
        Permission permission = userGroupPermission.getPermission();
  ...
  @SuppressWarnings("unchecked")
  @Transactional(readOnly = true)
  public List<User> findWithPermission(String permission) {
  ...
  @Transactional(readOnly = true)
  public Long getAdminUserCount(User user, UserGroup userGroup) {
  ...
  public boolean canDelete(User user) {
    return (getAdminUserCount(user, null) > 0) && (user.getTimes().size() == 0);  
  }
  ...
  public void delete(String id) {
    ...
    user.getUserTimeCustomers().clear();
