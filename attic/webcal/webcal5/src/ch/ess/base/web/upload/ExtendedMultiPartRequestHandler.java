...
public class ExtendedMultiPartRequestHandler implements MultipartRequestHandler {
  ...
  public void handleRequest(HttpServletRequest request) throws ServletException {
    ...
    FileItemFactory factory = new MonitoredDiskFileItemFactory((int)getSizeThreshold(ac), getRepositoryPath(ac),
        listener);
    ...
    //    upload.setSizeMax((int)getSizeThreshold(ac));
   ...
   */
  protected long convertSizeToBytes(final String sizeString, final long defaultSize) {
    int multiplier = 1;
    ...
    if (multiplier != 1) {
      sizeStr = sizeString.substring(0, sizeString.length() - 1);
    }
    ...
    try {
      ...
      log.warn("Invalid format for file size ('" + sizeStr + "'). Using default.");
      size = defaultSize;
      multiplier = 1;
    }
   ...
   */
  protected File getRepositoryPath(ModuleConfig mc) {
