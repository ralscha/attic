...
<%  } else {%>
	  <td align="center" bgcolor="#FFFFFF" class="textsmall"><%= ch.ess.calendar.util.Constants.SHORT_WEEKDAYS[tmp.get(Calendar.DAY_OF_WEEK)] %></td>    
<%  } 
  }
%>
...
<% if (login.isLogonOK()) {
	 ...
	 <% } else {%>
		  <td align="center" bgcolor="#FFFFFF" ><a href="d.jsp?onedate=<%= ch.ess.calendar.util.Constants.dateFormat.format(tmp.getTime()) %>&startdate=<%= ch.ess.calendar.util.Constants.dateFormat.format(tmp.getTime()) %>&month=<%=summary.getMonth()%>&year=<%= summary.getYear()%>"><%= i %></a></td>    
	 <% }
     }
   } else {
		for (int i = 1; i <= summary.getLastDay(); i++) {
