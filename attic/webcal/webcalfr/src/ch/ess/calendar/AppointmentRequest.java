...
public class AppointmentRequest {
  ...
  private void sendEmail(Appointments app, String toemail, String from) {    
    ...
    msg.append("Nouvel événement: ").append(app.getSubject()).append("\n");
    msg.append("(" + from + ")");
    msg.append("\n");
    msg.append("\n");
    ...
    mailSender.sendMail(sender, toemail, 
                        "Nouvel événement: " + app.getSubject(), msg.toString());
  ...
  private String getCategoryDescription(int id) { 
	...
	public boolean inputValidation() {
		if (isNullString(title)) {
			errorText = "Prière de saisir un titre";
			return false;
		}
		
		if (mode == ONE_MODE) {
			if (isNullString(onedate)) {
				errorText = "Prière de saisir une date";
				return false;
			}
			...
			try {
				...
				errorText = "Format de la date incorrect: "+onedate;
				return false;
			}
						
			if (event == TIMED_EVENT) {
				if (isNullString(starttime) || isNullString(endtime)) {
					errorText = "Prière de saisir une date de début et de fin";
					return false;
				}
          ...
          try {
            ...
            try {
              ...
              if ((hour >= 0) && (hour <= 23)) {
                ...
                errorText = "Heure de début incorrecte: " + starttime;
                return false;
              }
              
            } catch (NumberFormatException nfe) {
    					errorText = "Format de date de début incorrect: "+starttime;
  	  				return false;
            }
  				}
  			}
          ...
          try {
            ...
            try {
              ...
              if ((hour >= 0) && (hour <= 23)) {
                ...
                errorText = "Heure de fin incorrecte: " + endtime;
                return false;
              }            
            } catch (NumberFormatException nfe) {
    					errorText = "Format d'heure de fin incorrect: "+endtime;  
    					return false;
            }
          }
				}
			...
			if (isNullString(startdate)) {
				errorText = "Prière de saisir une date de début";
				return false;
			}
			
			if (isNullString(enddate)) {
				errorText = "Prière de saisir une date de fin";
				return false;
			}
			...
			try {
				...
				errorText = "Format de date de début incorrect: "+startdate;
				return false;
			}
			...
			try {
				...
				errorText = "Format de date de fin incorrect: "+startdate;
				return false;
			}
