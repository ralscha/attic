...
public class RecurranceRuleRfc implements RecurranceRule {
  ...
  protected Set<Integer> secSet = new HashSet<Integer>();
  protected Set<Integer> minSet = new HashSet<Integer>();
  protected Set<Integer> hourSet = new HashSet<Integer>();
  protected Set<ByDay> weekDaySet = new HashSet<ByDay>();
  protected Set<Integer> monthDaySet = new HashSet<Integer>();
  protected Set<Integer> yearDaySet = new HashSet<Integer>();
  protected Set<Integer> yearSet = new HashSet<Integer>();
  protected Set<Integer> weekNumberSet = new HashSet<Integer>();
  protected Set<Integer> monthSet = new HashSet<Integer>();
  protected Set<Integer> bspSet = null;

  protected List<Calendar> resultList = null;

  protected List<Calendar> protoDateList = new ArrayList<Calendar>();
  ...
  RecurranceRuleRfc(String ruleI, Calendar startI, Calendar endI) {
    try {
      ...
      if (untilDate != null) {
        if ((endDate == null) || ((endDate != null) && (endDate.after(untilDate)))) {
          endDate = untilDate;
        }
      }
      ...
      cur.get(Calendar.WEEK_OF_YEAR);
      //cur.setLenient(false);                  

      Calendar limit = (Calendar)endDate.clone();
      limit.get(Calendar.WEEK_OF_YEAR);
      ...
      for (int pdCtr = protoDateList.size() - 1; pdCtr >= 0; pdCtr--) {
        Calendar curProtoDate = protoDateList.get(pdCtr);
      ...
      for (int pdCtr = protoDateList.size() - 1; pdCtr >= 0; pdCtr--) {
        ...
        Set<Calendar> resultPile = new HashSet<Calendar>();
        resultPile.add(curProtoDate);
        if (resultPile.size() > SET_SIZE_LIMIT) {
          throw new IllegalArgumentException(
              "Recurrance too large.  Try moving your end date forward.  For example, if you are doing a MINUTELY recurrance over 5 years, you will get this error even in your COUNT is set to 2.  Working with a smaller set relieves this problem.");
        }
      ...
      for (int rctr = resultList.size() - 1; rctr >= 0; rctr--) {
        Calendar curDate = resultList.get(rctr);
      ...
      if (offsetAmount != 0) {
        for (int ctr = 0; ctr < finalList.size(); ctr++) {
          Calendar curMod = finalList.get(ctr);
      ...
      resultList = new ArrayList<Calendar>(finalList);
   ...
   ***/
  public List<Calendar> getAllMatchingDatesOverRange(Calendar start, Calendar end) {
    return null;
  }
   ...
   ***/
  public List<Calendar> getAllMatchingDates() {
    return resultList;
  }
   ...
   ***/
  private Set<Integer> getNumberSet(String list, int low, int high, String key) {
   ...
   ***/
  private Set<Integer> getNumberSetHoldTheEvil(String list, int low, int high, String key) {
    Set<Integer> old = getNumberSet(list, low, high, key);
    Set<Integer> result = new HashSet<Integer>();

    for (int oldValue : old) {
      result.add(oldValue - 1);
    }
   ...
   ***/
  private Set<ByDay> getWeekDaySet(String list) {
    ...
    while (stk.hasMoreTokens()) {
      ...
      if (bd.count != 0 && !("MONTHLY".equals(freq) || "YEARLY".equals(freq))) {
        throw new IllegalArgumentException(
            "  ERROR: You can only specify a number with a week day in the YEARLY or MONTHLY frequencies.");
      }
      result.add(bd);
    }
    return result;
  ...
  /***
   ...
   @param end the end of the range.
   ***/
  private List<Calendar> getFullDateValueList(int unit, int intervalL, Calendar inputCur, Calendar inputEnd) {
    Calendar cur = roundDown(unit, (Calendar)inputCur.clone());
    Calendar end = roundDown(unit, (Calendar)inputEnd.clone());
    ...
    List<Calendar> resultListL = new ArrayList<Calendar>();
    resultListL.add((Calendar)cur.clone());
    LOGGER.debug("val " + cur.getTime());

    cur.add(unit, intervalL);

    while (CalendarUtil.isBeforeOrEqual(cur, end)) { // while not past the end date
      LOGGER.debug("val " + cur.getTime());
      resultListL.add((Calendar)cur.clone());
      cur.add(unit, intervalL);
      if (resultListL.size() > SET_SIZE_LIMIT) {
        throw new IllegalArgumentException(
            "Recurrance too large.  Try moving your end date forward.  For example, if you are doing a MINUTELY recurrance over 5 years, you will get this error even in your COUNT is set to 2.  Working with a smaller set relieves this problem.");
      }
    }
  ...
  private Calendar roundDown(int unit, Calendar unRounded) {
    ...
    if (unit == Calendar.WEEK_OF_YEAR) {
      int week = unRounded.get(Calendar.WEEK_OF_YEAR);
      int month = unRounded.get(Calendar.MONTH);

      if ((week == 1) && (month == Calendar.DECEMBER)) {
        result.set(Calendar.YEAR, unRounded.get(Calendar.YEAR) + 1);
      }

      //System.out.println(ch.ess.cal.Constants.DATE_UTC_FORMAT.format(result));
   ...
   ***/
  private boolean isDateInMetaSet(Calendar curProtoDate, int unit, Set<Integer> metaSet) {
    int cur = curProtoDate.get(unit);
    Set<Integer> locallyValidSet = new HashSet<Integer>(metaSet);

    int localMax = curProtoDate.getActualMaximum(unit);

    for (int modVal : locallyValidSet) {
      if (modVal < 0) {
        //-1 implies the last item in the group, so the +1 makes it all work out.
        locallyValidSet.add(modVal + localMax + 1);
      }
    }
  ...
  /***
   ...
   @return true iff the date is part of the meta set
   ***/
  private boolean isDateInWeekDayMetaSet(Calendar curProtoDate, Set<ByDay> metaSet) {
    for (ByDay bd : metaSet) {
  ...
  private Calendar huntDayOfWeek(Calendar curProtoDate, ByDay bd) {
    ...
    int foundCountL = 0;
    int safety = 0;
    while (foundCountL < Math.abs(bd.count)) {
      ...
      if (safety > 366) {
        ...
        throw new IllegalArgumentException(
            "  ERROR: This loop has spun out of control. Either the week code you entered is bad, or you have asked for too large of an ofset");
      }
    }
   ...
   ***/
  private Set<Calendar> breedMetaSet(Set<Calendar> origSet, int unit, Set<Integer> metaSet) {
    ...
    for (Calendar curProtoDate : origSet) {

      int localMax = curProtoDate.getActualMaximum(unit);

      for (Integer modVal : metaSet) {
        if (modVal < 0) {
          //-1 implies the last item in the group, so the +1 makes it all work out.
  ...
  /***
   ...
   @return the now more populous set
   ***/
  private Set<Calendar> breedWeekDayMetaSet(Set<Calendar> origSet, Set<ByDay> metaSet, Calendar start, Calendar end) {
            ...
            if (!needSameMonth || (needSameMonth && isSameMonth(hunter, curProtoDate))) {
              if (!needSameWeek || (needSameWeek && isSameWeek(hunter, curProtoDate))) {
                if (!needSameDay || (needSameDay && isSameDay(hunter, curProtoDate))) {
                  result.add((Calendar)hunter.clone());
                  if (result.size() > SET_SIZE_LIMIT) {
                    throw new IllegalArgumentException(
                        "Recurrance too large.  Try moving your end date forward.  For example, if you are doing a MINUTELY recurrance over 5 years, you will get this error even in your COUNT is set to 2.  Working with a smaller set relieves this problem.");
                  }
    ...
    if (hunter.get(Calendar.WEEK_OF_YEAR) == curProtoDate.get(Calendar.WEEK_OF_YEAR)) {
      if ((curProtoDate.get(Calendar.MONTH) == Calendar.DECEMBER) && (curProtoDate.get(Calendar.WEEK_OF_YEAR) == 1)) {
        ...
        if ((hunter.get(Calendar.MONTH) == Calendar.DECEMBER)
            || ((hunter.get(Calendar.MONTH) == Calendar.JANUARY) && (hunter.get(Calendar.YEAR) == curProtoDate
                .get(Calendar.YEAR) + 1))) {
          return true;
        }
        return false;
   ...
   ***/
  public List<Calendar> filterUsingBySetPos(List<Calendar> inputList, Set<Integer> bspSetL, int freqNumber) {
    LOGGER.debug("in bsp");
    //--- sanity checking
    if (inputList == null) {
      return null;
    ...
    for (int ctr = 0; ctr < inputList.size(); ctr++) {
      ...
      if (curCal.get(freqNumber) == curVal) {
        ...
        newResultList.addAll(getSetPosItems(bspSetL, curList));
        curList.clear();
        curList.add(inputList.get(ctr));
  ...
  public List<Calendar> getSetPosItems(Set<Integer> controlSet, List<Calendar> values) {
    ...
    while (it.hasNext()) {
      ...
      try {
        if (i >= 0) {
          ...
          resultListL.add(values.get(i - 1));
        } else {
          //-- take the nth to the last
          resultListL.add(values.get(values.size() + i));
        }
