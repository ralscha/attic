...
public class Recurrance {
  private List<RecurranceRuleRfc> ruleList = null;
  private List<Calendar> resultList = null;
  ...
  public Recurrance(String rule, Calendar start, Calendar end) {
    ruleList = new ArrayList<RecurranceRuleRfc>();
  ...
  public List getAllMatchingDatesOverRange(Calendar start, Calendar end) {
    ...
    List<Calendar> all = getAllMatchingDates();

    for (int ctr = 0; ctr < all.size(); ctr++) {
      Calendar cur = all.get(ctr);
  ...
  public List<Calendar> getAllMatchingDates() {
    if (resultList == null) {
      ...
      for (RecurranceRule tmpRule : ruleList) {
        List<Calendar> tmpList = tmpRule.getAllMatchingDates();

        for (Calendar c : tmpList) {
  ...
  public String getRule() {
    ...
    for (RecurranceRule rule : ruleList) {
      if (str.length() > 0) {
        str += "/";
      }
      str += rule.getRule();
  ...
  private boolean contains(List<Calendar> list, Calendar value) {
  ...
  public RuleDescription getRuleDescription(int ix) {
    RecurranceRule rr = ruleList.get(ix);
    return rr.getRuleDescription();
