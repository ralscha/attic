...
public class CategoryListAction extends HibernateAction {
  ...
  public ActionForward execute(
    ...
    if (name != null) {
      ...
      resultList =
        sess.find(
          "from Category as category where category.name like ? order by category.name asc",
          name,
          Hibernate.STRING);
    } else {
      resultList = sess.find("from Category as category order by category.name asc");
    }
