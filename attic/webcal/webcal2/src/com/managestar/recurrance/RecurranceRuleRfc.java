...
public class RecurranceRuleRfc implements RecurranceRule {
  ...
  ***/
  ...
  @param end the end of the range.
  ***/
  private List getFullDateValueList(int unit, int intervalL, Calendar inputCur, Calendar inputEnd) {
    Calendar cur = roundDown(unit, (Calendar)inputCur.clone());
    Calendar end = roundDown(unit, (Calendar)inputEnd.clone());
    ...
    List resultListL = new ArrayList();
    resultListL.add(cur.clone());
  ...
  ***/
  public List filterUsingBySetPos(List inputList, Set bspSetL, int freqNumber) {
    LOGGER.debug("in bsp");
    //--- sanity checking
    if (inputList == null) {
      return null;
