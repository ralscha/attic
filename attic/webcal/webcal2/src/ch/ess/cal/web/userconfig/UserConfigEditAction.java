...
public class UserConfigEditAction extends DispatchAction {
  ...
  protected ActionForward store() throws Exception {
    ...
    Map params = WebUtils.getChangeRequestParameterMap(request);

    if (EmailUtil.handleEmailRequest(params, prefForm.getEmail(), prefForm.getEmails())) {
      if (params.keySet().contains("addemail")) {
        prefForm.setEmail(null);
      }
