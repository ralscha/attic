...
public final class LoginServlet extends HttpServlet {
        ...
        if (request.getParameter("newPassword") != null) {
          ...
          if (user != null) {
            ...
            if (!GenericValidator.isBlankOrNull(remember)) {
              ...
              Cookie newCookie = new Cookie(ch.ess.cal.Constants.COOKIE_REMEMBER, user.getLoginToken());
  ...
  private void sendPasswordMail(User user, String newPassword) throws HibernateException {

    Collection email = HibernateSession.currentSession().filter(user.getEmails(), "where defaultEmail = true");
    if (!email.isEmpty()) {
      Email em = (Email)email.iterator().next();
