  ...
  private Reader_writer lock = new Reader_writer();
    ...
    try {
      ...
      query.addOrder("firstname");
	    ...
	    Users[] users = (Users[])db.list(Users.class, query);			
			
			
			query.reset();
			query.addOrder("enddate");
			query.add("group", "userid", OQuery.EQUAL);
      Appointments[] apps = (Appointments[])db.list(Appointments.class, query);
			...
			if (apps != null) {
			  for (int i = 0; i < apps.length; i++) {  				
  				groupAppointmentsList.add(apps[i]);
  				appointmentsMap.put(apps[i].getAppointmentid(), apps[i]);
  			}
			}
      ...
      query.addOrder("user");
      query.addOrder("enddate");
      query.add("group", "userid", OQuery.NOT_EQUAL);
      apps = (Appointments[])db.list(Appointments.class, query);   
      ...
      if (apps != null) {
        ...
        List t = (List)tmp.get(apps[i].getUser().getUserid());
        if (t == null) {
          t = new ArrayList();
          tmp.put(apps[i].getUser().getUserid(), t);
        }
        ...
        t.add(apps[i]);
			...
			if (users != null) {
			 for (int i = 0; i < users.length; i++) {
				...
				List appointmentList = (List)tmp.get(users[i].getUserid());
				if (appointmentList == null) {
				  appointmentList = new ArrayList();
				}
        userMap.put(users[i].getUserid(), appointmentList);   
      }
			}	
		} finally {
