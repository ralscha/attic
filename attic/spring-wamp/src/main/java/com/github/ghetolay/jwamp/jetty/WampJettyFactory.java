...
public class WampJettyFactory extends WampFactory {
	...
	@Override
	protected WampConnection getConnection(URI uri, long timeout,
			ReconnectPolicy reconnectPolicy, Collection<WampMessageHandler> handlers,
			ResultListener<WampConnection> wr) throws TimeoutException, Exception {
		JettyConnection connection = new JettyConnection(uri, getObjectMapper(),
				handlers, wr);
	...
	protected void connect(URI uri, long timeout, JettyConnection connection)
			throws Exception {
		if (!fact.isStarted()) {
			fact.start();
		}
	...
	public WampJettyHandler newJettyHandler(WampParameter param,
			WampWebSocketListener listener) {
