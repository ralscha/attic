...
public class SimpleRPCManager implements WampMessageHandler {
	...
	private final ActionMapping<CallAction> callMapping;
	...
	public void processAction(WampCallMessage callMsg) {
		...
		if (log.isDebugEnabled()) {
			log.debug("Processing Call " + callMsg.getProcId() + " with id "
					+ callMsg.getCallId());
		}
		...
		if ((action = callMapping.getAction(callMsg.getProcId())) != null) {
			try {
				...
				WampCallErrorMessage errorMsg = new WampCallErrorMessage(
						callMsg.getCallId(), callMsg.getProcId(), e.getLocalizedMessage());
			...
			resultMsg = new WampCallErrorMessage(callMsg.getCallId(),
					callMsg.getProcId(), "Action nor found");
		}
		...
		if (resultMsg != null) {
			try {
				...
				if (log.isErrorEnabled()) {
					log.error("Enable to send message in response of Call "
							+ callMsg.getProcId() + " with id " + callMsg.getCallId());
				}
			}
		}
	}
