...
/**
 * A WampConnection is a WebSocket connection with Wamp attributes. Each connection is
 * identified by it's session Id. The messages are handled by {@link WampMessageHandler}.
 *
 * @author ghetolay
...
public interface WampConnection {
	/**
	 *
	 * Enum for the reconnect policy. It's a boolean with a third value : IMPOSSIBLE,
	 * because in some case it can be impossible to reconnect. For example when it's a
	 * client connection initiate from the other side.
	 *
	 ...
	 *
	 */
	public static enum ReconnectPolicy {
		YES, NO, IMPOSSIBLE
	};

	/**
	 * Send a {@link WampMessage} to the other side of the connection. This method should
	 * not be called directly, use {@link WampWebSocket} methods or a specific {@Link
	 *  WampMessageHandler} instead.
	 *
	...
	/**
	 * Set whether we should stop at the first successful handler or not. So if multiple
	 * handlers are handling the same message type, exclusiveHandler should be false.
	 *
	 ...
	 * @return If the set was successful i.e. if the redirect is not
	 * {@link WampConnection.ReconnectPolicy#IMPOSSIBLE}
	...
	/**
	 * Return the first handler of type handlerClass. This method should be deprecated
	 * soon.
	 *
