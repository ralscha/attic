...
public class WampPublishMessage extends WampMessage {
	...
	public WampPublishMessage(Object[] JSONArray) throws BadMessageFormException {
		...
		if (JSONArray.length < 3) {
			throw BadMessageFormException.notEnoughParameter("Publish", JSONArray.length,
					3);
		}
		...
		try {
			if (parser.nextToken() != JsonToken.VALUE_STRING) {
				throw new BadMessageFormException(
						"TopicUri is required and must be a string");
			}
			...
			if (parser.nextToken() != JsonToken.END_ARRAY) {
				if (parser.getCurrentToken() == JsonToken.VALUE_TRUE) {
					...
					while (parser.nextToken() != JsonToken.END_ARRAY) {
						if (parser.getCurrentToken() != JsonToken.VALUE_STRING) {
							throw new BadMessageFormException(
									"Fourth element must be boolean or array of string");
						}
				...
				if (parser.nextToken() != JsonToken.END_ARRAY) {
					...
					while (parser.nextToken() != JsonToken.END_ARRAY) {
						if (parser.getCurrentToken() != JsonToken.VALUE_STRING) {
							throw new BadMessageFormException(
									"Fifth element must be an array of string");
						}
	...
	public <T> T getEvent(Class<T> c) throws Exception {
		...
		if (event instanceof Map<?, ?>) {
			...
			for (Entry<String, Object> entry : eventMap.entrySet()) {
				c.getMethod("set" + entry.getKey(), entry.getValue().getClass()).invoke(
