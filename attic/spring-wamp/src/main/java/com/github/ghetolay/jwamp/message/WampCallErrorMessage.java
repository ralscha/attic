...
public class WampCallErrorMessage extends WampCallResultMessage {
	...
	public WampCallErrorMessage(Object[] JSONArray) throws BadMessageFormException {
		...
		if (JSONArray.length < 4) {
			throw BadMessageFormException.notEnoughParameter("CallError",
					JSONArray.length, 4);
		}
		...
		try {
			if (parser.nextToken() != JsonToken.VALUE_STRING) {
				throw new BadMessageFormException(
						"CallId is required and must be a string");
			}
			setCallId(parser.getText());

			if (parser.nextToken() != JsonToken.VALUE_STRING) {
				throw new BadMessageFormException(
						"ErrorUri is required and must be a string");
			}
			setErrorUri(parser.getText());

			if (parser.nextToken() != JsonToken.VALUE_STRING) {
				throw new BadMessageFormException(
						"ErrorDescription is required and must be a string");
			}
