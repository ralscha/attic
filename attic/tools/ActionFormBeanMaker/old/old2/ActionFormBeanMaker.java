...
public class ActionFormBeanMaker {
  ...
  private void jsp2bean(String fileName, boolean validating) throws IOException {
    //parse jsp document
    ...
    for (int i = 0; i < forms.size(); i++) {
      ...
      if (typeAttr != null) {
        SourceFile sf = new SourceFile(typeAttr, validating);
  ...
  private void html2bean(String fileName, boolean validating) throws IOException {
    //Parse html document
    FileReader fr = new FileReader(fileName);
    TagTiller tiller = new TagTiller(fr);
    Tag theParsedPage = tiller.getTilledTags();
    ...
    for (int i = 0; i < forms.size(); i++) {
      ...
      if (nameAttr != null) {
        SourceFile sf = new SourceFile(nameAttr, validating);
  ...
  public static void main(String[] args) {
    if (args.length >= 2) {
      try {
        ...
        if (htmlPage) {
          new ActionFormBeanMaker().html2bean(fileName, validating);
        } else {
          new ActionFormBeanMaker().jsp2bean(fileName, validating);
        }
