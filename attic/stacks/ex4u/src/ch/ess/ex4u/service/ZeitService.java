...
public class ZeitService {
  ...
  Logger log = new Logger(ZeitService.class.getName());
  ...
  public DSResponse fetch(DSRequest dsRequest) throws Exception {
    ...
    Criterion zeitRestriction = null;
    if (userName != null) {
      zeitRestriction = Restrictions.like("userName", userName, MatchMode.ANYWHERE);
      criteria.add(zeitRestriction);
    }
    ...
    criteria = hibernateSession.createCriteria(Zeit.class);
    if (userName != null)
      criteria.add(zeitRestriction);
  ...
  @Transactional
  public DSResponse add(DSRequest dsRequest, Zeit item) throws Exception {
  ...
  public DSResponse update(DSRequest dsRequest, Map newValues) throws Exception {
    ...
    Session hibernateSession = (Session)entityManager.getDelegate();
    Zeit item = (Zeit)hibernateSession.get(Zeit.class, id);
  ...
  @Transactional
  public Zeit remove(Zeit item) throws Exception {
    Session hibernateSession = (Session)entityManager.getDelegate();
    Zeit user = (Zeit)hibernateSession.get(Zeit.class, item.getId());
