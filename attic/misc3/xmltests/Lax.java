...
/**
 * Parse input XML with ShoppingListWriter
 * @param args java.lang.String[]
 */
public static void main(String args[]) {
	if (args.length < 1) {
		System.err.println("Usage: lax inputFile.xml [parserClass]");
		System.exit(1);
	}

	String sInputFile = args[0];
	String sRecipeFile;
	String sShoppingListFile;
	String sBase = sInputFile;
	
	if (sBase.length() > 4 && sBase.toLowerCase().endsWith(".xml")) {
		sBase = sBase.substring(0, sBase.length() - 4);
	} else {
		sInputFile = sBase + ".xml";
	}
	sRecipeFile = sBase + "-recipe.html";
	sShoppingListFile = sBase + "-list.html";

	Lax lax = new Lax();

	ShoppingListWriter slw = new ShoppingListWriter(sShoppingListFile);
	lax.addHandler(slw);
	
	RecipeWriter rw = new RecipeWriter(sRecipeFile);
	lax.addHandler(rw);

	lax.parseDocument(true, lax, sInputFile);
