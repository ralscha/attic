...
public class AWZACheck {

	private Map bookingsMap = new HashMap();
	...
	public void check(String fileName) {
		String line;
		String macct = null;
		com.ibm.math.BigDecimal gtotal = com.ibm.math.BigDecimal.ZERO;
		
		try {
			String okFileName = AppProperties.getStringProperty("awza.check.ok.file");
			...
			String realBalanceFileName = AppProperties.getStringProperty("awza.check.balance.file");
			...
			String openItemsFileName = AppProperties.getStringProperty("awza.check.diff.html.prefix");			
			String openItemsFilePath = AppProperties.getStringProperty("awza.check.diff.html.path");
			String ovFile = AppProperties.getStringProperty("awza.check.overview.html.file");
			String okDate = AppProperties.getStringProperty("awza.check.ok.date");
				...
				if (line.length() >= 109) {
					try {
						...
						if (okDate.compareTo(obj.getOrderDate()) < 0) {
							...
							obj.setAmount(new com.ibm.math.BigDecimal(line.substring(86, 103).trim()));		
							...
							if (macct == null)
								...
								specialList.clear();
								bookingsMap.clear();
								gtotal = com.ibm.math.BigDecimal.ZERO;
	...
	public static void main(String args[]) throws Exception {
		...
		if (args.length == 1) {
			new AWZACheck().check(args[0]);
		} else {
			String propsFileName = System.getProperty("gtf.props.file");
			if (propsFileName != null) 
				AppProperties.setFileName(propsFileName);
			else
				AppProperties.setFileName("gtf.props");
			new AWZACheck().check(AppProperties.getStringProperty("awza.check.bookings.file"));
		}
	}
			...
			if (total.compareTo(zero) != 0) {                    
				...
				if (speB == null) {
					...
					while(itm.hasNext()) {
						...
						total = com.ibm.math.BigDecimal.ZERO;
	...
	private void printHeader(PrintWriter pw, Calendar lastUpdate) {
		...
		pw.println("<FORM ACTION=\""+AppProperties.getStringProperty("awza.check.servlet")+
								"\" METHOD=\"POST\">");
	...
	private void printOverviewRow(PrintWriter pw, String account, String link, 
								com.ibm.math.BigDecimal diff, int valueDate, String currency) {
		pw.println("<tr>");
		pw.print("<td><a href=\"");
		pw.print(link);
		pw.print("\">");
	...
	private void printSpecialList(PrintWriter pw) {
		...
		if (specialList.size() > 0) {
			...
			if (total.compareTo(com.ibm.math.BigDecimal.ZERO) == 0) 
				return;
	...
	private void printTotal(PrintWriter pw, com.ibm.math.BigDecimal total) {
		pw.println("<tr>");
	...
	private Booking searchSpecialList(com.ibm.math.BigDecimal amount) {
	...
	private boolean searchSpecialList(com.ibm.math.BigDecimal amount, String orderDate) {
		List l = new Vector();
		com.ibm.math.BigDecimal total = com.ibm.math.BigDecimal.ZERO;
		for (int i = 0; i < specialList.size(); i++) {
			Booking b = (Booking) specialList.get(i);
