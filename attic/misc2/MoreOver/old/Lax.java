...
public class Lax extends DefaultHandler {
	 ...
	 * current tag.
	 */
	public void characters(char[] caChars, int iStart, int iLength) throws SAXException {
		...
		if (sCurrentTag != null) {
			...
			for (i = 0; i < _vecHandlers.size(); i++) {
				...
				if (mTextMethod != null) {
					try {
						if (sArg == null) {
							sArg = new String(caChars, iStart, iLength);
						}
  ...
  public void endElement(String namespaceURI, String localName,
	 ...
	 */
	public boolean parseDocument(boolean isValidating, ContentHandler handler, InputSource inputSource) {
		try {
			...
			if (mStartMethod != null) {
				try {
					...
					if (caMethodArgs.length == 0) {
						...
						mStartMethod.invoke(oThisHandler, new Object[] {atts});
					}
				} catch (InvocationTargetException ex) {
					System.err.println(ex);
				} catch (IllegalAccessException ex) {
