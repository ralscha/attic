...
public class Phonet {
	...
	private final static int ANZ_HASH = 256;
	...
	private void load() {
		...
		try {
			pattern = compiler.compile("^[ ]*\"([^\"]*||\")\",[ ]*(?:\"([^\"]*)\"||NULL),[ ]*(?:\"([^\"]*)\"||NULL),");
		...
		try {
			BufferedReader br = new BufferedReader(
                      			new InputStreamReader(getClass().getResourceAsStream("rules.dat")));
			String line;
			...
			while ((line = br.readLine()) != null) {
				if (matcher.contains(line, pattern)) {
					...
					String rule1 = result.group(2);
					String rule2 = result.group(3);
	...
	public String phonet(String wort, int modus) {
	...
	public static void main(String args[]) {
		...
		String phon = new Phonet().phonet(text, 1);
    System.out.println();
		
		System.out.println("Umwandlung nach 1. Regelsatz:  \""+phon+"\"");
		
		phon = new Phonet().phonet(text, 2);
