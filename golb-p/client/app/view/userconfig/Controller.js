Ext.define('Golb.view.userconfig.Controller', {
	extend: 'Ext.app.ViewController',

	initViewModel: function() {
		var userSettings = new Golb.model.UserSettings();
		userSettings.load({
			scope: this,
	...
	save: function() {
		...
		userSettings.save({
			scope: this,
			success: function(record, operation) {
				Golb.Util.successToast(i18n.savesuccessful);
			},
			failure: function(record, operation) {
				Golb.Util.errorToast(i18n.inputcontainserrors);
				var form = this.lookup('userConfigForm').getForm();
				var validations = operation.getResponse().result.validations;
				Golb.Util.markInvalidFields(form, validations);
			},
	...
	clearState: function() {
		localStorage.removeItem("golb_navigation_micro");
	...
	destroyPersistentLogin: function(view, rowIndex, colIndex, item, e, record, row) {
		record.erase({
			callback: function() {
				Golb.Util.successToast(i18n.destroysuccessful);
			}
		});
	},
	...
	enable2f: function() {
		...
		userConfigService.enable2f(function(secret) {
			Golb.Util.successToast(i18n.userconfig_2fa_enabled);
	...
	disable2f: function() {
		...
		userConfigService.disable2f(function() {
			Golb.Util.successToast(i18n.userconfig_2fa_disabled);
