Ext.define('Golb.view.auth.AuthController', {
		...
		Golb.Util.getCsrfToken().then(function() {
			form.submit({
				...
				failure: function(form, action) {
					Golb.Util.errorToast(i18n.auth_signin_failed);
				}
			});
		});
	},
	...
	onForgotPasswordAfterRender: function(link) {
		link.el.on('click', function() {
			...
			if (val) {
				sessionStorage.golb_login = val;
			}
			else {
				sessionStorage.removeItem('golb_login');
			}
		});
	},
	...
	onLoginResumeButtonClick: function() {
		...
		form.submit({
			...
			failure: function(form, action) {
				Golb.Util.errorToast(i18n.auth_locked_resume_failed);
			}
		});
	},
	...
	onResetClick: function() {
		...
		var token = Golb.app.pwResetToken;
