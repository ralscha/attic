...
public class EventBus {
	...
	public EventBus(ObjectMapper objectMapper) {
		this.clients = ExpiringMap.builder().expiration(1, TimeUnit.DAYS).expirationListener(this::expirationListener)
				.build();
		this.events = new ConcurrentHashMap<>();
		this.objectMapper = objectMapper;
	}

	private void expirationListener(final String clientId, final EventBusClient client) {
	...
	public void handleEvent(EventBusEvent event) {
		...
		for (String clientId : clientIds) {
			...
			if (client != null) {
				try {
					client.emitter().send(SseEmitter.event().name(event.name()).data(data));
				} catch (Exception e) {
