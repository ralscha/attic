...
public abstract class User {
	...
	public static User create(@JsonProperty("_id") String id,
			@JsonProperty("index") int index, @JsonProperty("guid") String guid,
			@JsonProperty("is_active") boolean isActive,
			@JsonProperty("balance") String balance,
			@JsonProperty("picture") String pictureUrl, @JsonProperty("age") int age,
			@JsonProperty("name") Name name, @JsonProperty("company") String company,
		...
		return builder().id(id).index(index).guid(guid).isActive(isActive)
				.balance(balance).pictureUrl(pictureUrl).age(age).name(name)
				.company(company).email(email).address(address).about(about)
				.registered(registered).latitude(latitude).longitude(longitude).tags(tags)
				.range(range).friends(friends).images(images).greeting(greeting)
				.favoriteFruit(favoriteFruit).eyeColor(eyeColor).phone(phone).build();
