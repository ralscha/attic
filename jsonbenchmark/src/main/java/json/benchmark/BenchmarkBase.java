 ...
 * @see <a href="http://java-performance.info/introduction-jmh-profilers/"> Introduction
 * to JMH Profilers</a>
 */
public class BenchmarkBase {
	...
	public static class ParserBenchmarkBase extends BenchmarkBase {
		...
		public void findResource() throws Exception {
			this.json = BenchmarkBase.class.getClassLoader()
					.getResource(this.type + "sample.json");
		}
	}
	...
	public static void runForDebug(Class<?> clazz) throws Exception {
		Options options = new OptionsBuilder().include(clazz.getCanonicalName())
				.warmupIterations(2).measurementIterations(2).forks(2).build();
		new Runner(options).run();
	}

	public static void main(String[] args) throws Exception {
		Options options = new OptionsBuilder().include(".*Benchmark").warmupIterations(2)
				.measurementIterations(2).forks(2).build();
		new Runner(options).run();
	}
}
