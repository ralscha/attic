...
public class AccessLogService {
	...
	public ExtDirectStoreResult<AccessLog> read(ExtDirectStoreReadRequest request,
		...
		JPQLQuery query = new JPAQuery(this.entityManager).from(QAccessLog.accessLog);
		...
		PeriodFormatter minutesAndSeconds = new PeriodFormatterBuilder().appendMinutes()
				.appendSuffix(" " + this.messageSource.getMessage("minute", null, locale),
						" " + this.messageSource.getMessage("minutes", null, locale))
				.appendSeparator(
						" " + this.messageSource.getMessage("and", null, locale) + " ")
				.printZeroRarelyLast().appendSeconds()
				.appendSuffix(" " + this.messageSource.getMessage("second", null, locale),
						" " + this.messageSource.getMessage("seconds", null, locale))
				.toFormatter();
	...
	public void deleteAll() {
		new JPADeleteClause(this.entityManager, QAccessLog.accessLog).execute();
	}
	...
	public void addTestData(HttpServletRequest request) {
		...
		for (int i = 0; i < 1000; i++) {
			try {
				...
				DateTime logIn = new DateTime(2011, random.nextInt(12) + 1,
