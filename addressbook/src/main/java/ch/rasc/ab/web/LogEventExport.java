...
public class LogEventExport {
	...
	public void logEventExport(HttpServletResponse response, @RequestParam(
			required = false) final String level) throws Exception {
		...
		try (OutputStream out = response.getOutputStream();
			...
			if (StringUtils.isNotBlank(level)) {
				events = new JPAQuery(entityManager).from(QLogEvent.logEvent)
						.where(QLogEvent.logEvent.level.eq(level))
						.list(QLogEvent.logEvent);
			}
			else {
				events = new JPAQuery(entityManager).from(QLogEvent.logEvent).list(
						QLogEvent.logEvent);
			}
