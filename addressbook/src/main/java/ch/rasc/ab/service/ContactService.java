...
import ch.rasc.ab.entity.QContact;
import ch.rasc.ab.entity.QContactGroup;
...
public class ContactService extends BaseCRUDService<Contact> {
	...
	public ExtDirectStoreResult<Contact> read(ExtDirectStoreReadRequest request) {
		...
		JPQLQuery query = new JPAQuery(entityManager).from(QContact.contact);
		...
		QueryUtil.addPagingAndSorting(query, request, Contact.class, QContact.contact);
	...
	protected void preModify(Contact entity) {
		if (entity.getContactGroupIds() != null) {
			...
			if (entity.getId() != null) {
				Contact dbContact = entityManager.find(Contact.class, entity.getId());
			...
			for (Long newGroupId : newGroupIds) {
				ContactGroup contactGroup = entityManager.getReference(
						ContactGroup.class, newGroupId);
				entity.getContactGroups().add(contactGroup);
			}

			for (Long removeGroupId : removeGroupIds) {
				ContactGroup contactGroup = entityManager.getReference(
						ContactGroup.class, removeGroupId);
